...
const ChatBot = () => {
  ...
  const ASTRA_DB_TOKEN = process.env.REACT_APP_ASTRA_DB_TOKEN || import.meta.env.VITE_ASTRA_DB_TOKEN || '';
  ...
  const langflowClient = new LangflowClient(
    process.env.NODE_ENV === 'production' ? 'https://blockbinge.vercel.app/api/langflow' : '/api/langflow',
    ASTRA_DB_TOKEN
  );

  const extractBotContent = (response) => {
    if (!response) return 'Unexpected response format.';
    const possiblePaths = [
    ...
    return possiblePaths.find((content) => content) || 'Could not parse AI response.';
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const botContent = extractBotContent(response);
      setMessages((prev) => [...prev, { type: 'bot', content: convertMarkdownToHtml(botContent), timestamp: new Date().toISOString() }]);
    } catch (error) {
      console.error('Chat Error:', error);
      setMessages((prev) => [...prev, { type: 'bot', content: `Error: ${error.message}`, timestamp: new Date().toISOString() }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <>
      <motion.button onClick={() => setIsOpen(true)} className="fixed bottom-4 right-4 bg-red-500 text-white p-4 rounded-full shadow-lg hover:bg-red-600 transition-all">
        <FaRobot className="text-2xl" />
      </motion.button>
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} transition={{ duration: 0.2 }} className="fixed bottom-4 right-4 w-96 h-[600px] bg-gray-900 rounded-2xl shadow-2xl flex flex-col">
            <div className="p-4 bg-red-600 text-white rounded-t-2xl flex justify-between">
              <h3 className="font-semibold flex items-center gap-2"><FaRobot /> Movie AI Assistant</h3>
              <motion.button onClick={() => setIsOpen(false)} className="hover:bg-white/20 p-2 rounded-full">
                <FaTimes />
              </motion.button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.map((msg, idx) => (
                <motion.div key={idx} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] p-3 rounded-2xl shadow-lg ${msg.type === 'user' ? 'bg-red-500 text-white' : 'bg-gray-800 text-gray-100'}`}>
                    <div className="text-xs opacity-75 flex items-center gap-2">
                      {msg.type === 'user' ? <FaUser /> : <FaRobot className="text-red-400" />} {new Date(msg.timestamp).toLocaleTimeString()}
                    </div>
                    <p className="text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: msg.content }}></p>
                  </div>
                </motion.div>
              ))}
              {isLoading && <motion.div className="text-gray-400">Typing...</motion.div>}
              <div ref={messagesEndRef} />
            </div>
            <div className="p-4 bg-gray-800">
              <div className="flex gap-2">
                <input ref={inputRef} type="text" value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="Ask about movies..." className="w-full bg-gray-700 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-red-500" disabled={isLoading} />
                <motion.button onClick={handleSendMessage} disabled={isLoading || !inputMessage.trim()} className="bg-red-500 text-white p-3 rounded-xl hover:bg-red-600">
                  <FaPaperPlane />
                </motion.button>
              </div>
            </div>
